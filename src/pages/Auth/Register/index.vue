<script setup>
import { ref } from 'vue'
import { AuthAPI } from 'src/api'
import { useRouter } from 'vue-router'
import UInput from 'src/components/ui/UInput.vue'
import UCheckbox from 'src/components/ui/UCheckbox.vue'
import UButton from 'src/components/ui/UButton.vue'
import { NotifyService } from 'src/services'

const router = useRouter()

const fieldsValues = ref({})

const isSubmitting = ref(false)
const submit = async () => {
  const { email, phone, firstName, lastName, password, confirmPassword } = fieldsValues.value

  if (password !== confirmPassword) {
    NotifyService.create({
      type: 'negative',
      message: 'Passwords must match',
    })
    return
  }

  isSubmitting.value = true
  try {
    await AuthAPI.register({ email, phone, firstName, lastName, password })
    router.replace({ name: 'billing' })
  } catch (error) {}
  isSubmitting.value = false
}
</script>

<template>
  <form
    class="tw-mx-auto tw-flex tw-h-full tw-w-[26.25rem] tw-flex-col tw-justify-center"
    @submit.prevent="submit"
  >
    <h1 class="page-title">Sign Up</h1>
    <p class="tw-mb-7.5 tw-mt-5 tw-text-dark-primary">Create account to start using Data Fusion.</p>
    <div class="tw-space-y-2.5">
      <UInput v-model="fieldsValues.email" type="email" placeholder="Email" required>
        <template #title>Email</template>
      </UInput>
      <UInput v-model="fieldsValues.phone" type="tel" placeholder="Phone number" required>
        <template #title>Phone number</template>
      </UInput>
      <UInput v-model="fieldsValues.firstName" placeholder="First name" required>
        <template #title>First name</template>
      </UInput>
      <UInput v-model="fieldsValues.lastName" placeholder="Last name" required>
        <template #title>Last name</template>
      </UInput>
      <UInput v-model="fieldsValues.password" type="password" placeholder="Password" required>
        <template #title>Password</template>
      </UInput>
      <UInput
        v-model="fieldsValues.confirmPassword"
        type="password"
        placeholder="Confirm password"
        required
      >
        <template #title>Confirm password</template>
      </UInput>
    </div>
    <UCheckbox class="tw-mb-7.5 tw-mt-5" required>
      I agree to
      <a
        class="underlined-link"
        href="https://cdn.prod.website-files.com/65e1bc1d4a342801c6df8e37/6706a10d68a03fcd695e8744_Data%20Fusion%20-%20Terms%20of%20Service%2010.03.2024%20LIVE.pdf"
        @click.stop
        >Terms of Service</a
      >
      and
      <a class="underlined-link" href="https://www.data-fusion.io/privacy" @click.stop
        >Privacy Policy</a
      >
    </UCheckbox>
    <UButton type="submit" class="tw-w-full" :loading="isSubmitting" rounded>
      Create your account
    </UButton>
    <p class="tw-my-7.5">
      Already a member?
      <router-link :to="{ name: 'login' }" class="underlined-link"> Login here </router-link>
    </p>
  </form>
</template>

<style lang="scss" scoped>
.underlined-link {
  @apply tw-text-primary tw-underline tw-decoration-primary tw-underline-offset-2 tw-transition-[text-decoration-color] hover:tw-decoration-transparent;
}
</style>
